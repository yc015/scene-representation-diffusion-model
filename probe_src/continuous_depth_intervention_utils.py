...
from probe_src.depth_loss import InverseDepthSmoothnessLoss
...
def load_classifiers_continuous_depth(at_step, weights_type="", interpolate=True):
    classifier_dict = {}
    if "no_bias" in weights_type:
        use_bias = False
    else:
        use_bias = True
    ...
    for block_type in ["down", "mid", "up"]:
        ...
        for block_ind in range(block_range_start, block_range):
            for layer_ind in range(layer_range):
                ...
                if "nonlinear" in weights_type:
                    probe = probeTwoNonLinearDense(input_dims_dict[f"{block_type}_{block_ind}"],
                                                   1,
                                                   scale_dict[f"{block_type}_{block_ind}"],
                                                   mid_channels=input_dims_dict[f"{block_type}_{block_ind}"] // 2,
                                                   use_bias=use_bias).to(torch_device)
                else:
                    probe = probeLinearDense(input_dims_dict[f"{block_type}_{block_ind}"],
                                             1,
                                             scale_dict[f"{block_type}_{block_ind}"], 
                                             use_bias=use_bias,
                                             interpolate=interpolate).to(torch_device)
                probe.load_state_dict(torch.load(f"probe_checkpoints/large_syn_dataset_continuous/at_step_{at_step}/regression_probe_{block_type}_{block_ind}_attn1_out_{layer_ind}{weights_type}.pth"))
...
def make_counterfactual_label(label, angle=0, translate=[0, 0], scale=1, sheer=0, fill=0):
    ...
    label_clone = functional.affine(torch.tensor(label_clone).to(torch.float).unsqueeze(0),
...
def optimize_one_inter_rep(inter_rep, layer_name, target, probe,
                           lr=1e-3, max_epoch=256, loss_func=nn.HuberLoss(),
                           smoothness_loss_func=None, image=None, verbose=False,
                           ):
