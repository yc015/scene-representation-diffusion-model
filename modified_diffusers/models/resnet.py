...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x, temb, hey=False):
        ...
        # if self.conv_1_mask:
        #     h *= self.conv_1_mask
        if hasattr(self.conv1, "conv_mask"):
            h = h * self.conv1.conv_mask 
        elif hasattr(self.conv1, "channels"):
            for c in self.conv1.channels:
                h[:, c, self.conv1.object_mask] = self.conv1.k
        ...
        # if self.conv_2_mask:
        #     h *= conv_2_mask
        if hasattr(self.conv2, "conv_mask"):
            h = h * self.conv2.conv_mask
        elif hasattr(self.conv2, "channels"):
            for c in self.conv2.channels:
                h[:, c, self.conv2.object_mask] = self.conv2.k
