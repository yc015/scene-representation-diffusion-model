...
class SpatialTransformer(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head, depth=1, dropout=0.0, context_dim=None, return_attn=False, return_qkv=False):
        super().__init__()
    ...
    def forward(self, x, context=None):
        ...
        for block in self.transformer_blocks:
            if self.return_attn:
                x, block_attn = block(x, context=context)
                attns.append(block_attn)
            elif self.return_qkv:
                x, block_qkv = block(x, context=context)
                qkvs.append(block_qkv)
            else:
...
class BasicTransformerBlock(nn.Module):
    def __init__(self, dim, n_heads, d_head, dropout=0.0, context_dim=None, gated_ff=True, checkpoint=True, return_attn=False, return_qkv=False):
        super().__init__()
        self.return_attn = return_attn
        self.return_qkv = return_qkv
    ...
    def forward(self, x, context=None):
        if self.return_attn:
            attns = {}
            attn_output, layer_attn = self.attn1(self.norm1(x))
            attns["self-attn"] = layer_attn
            x = attn_output + x
            attn_output, layer_attn = self.attn2(self.norm2(x), context=context)
            attns["cross-attn"] = layer_attn
            x = attn_output + x
            x = self.ff(self.norm3(x)) + x
            return x, attns
        elif self.return_qkv:
            qkvs = {}
            attn_output, layer_qkv = self.attn1(self.norm1(x))
            qkvs["self-attn"] = layer_qkv
            x = attn_output + x
            attn_output, layer_qkv = self.attn2(self.norm2(x), context=context)
            qkvs["cross-attn"] = layer_qkv
            x = attn_output + x
            x = self.ff(self.norm3(x)) + x
            return x, qkvs
        else:
...
class CrossAttention(nn.Module):
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0.0, return_attn=False, return_qkv=False):
        super().__init__()
        ...
        self.weight_attn_head = False
        self.attn_head_weights = None
        
        self.weight_out = False
        self.attn_out_weights = None
        
        self.weight_query_channel = False
        self.query_channel_weights = None
    ...
    def forward(self, x, context=None, mask=None):
        ...
        # Weight the attention head
        if self.weight_query_channel and self.query_channel_weights is not None:
            for (head, channel, weight) in sefl.query_channel_weights:
                q[head, ..., channel] *= weight
            self.weight_query_channel = False
        ...
        # Weight the attention map
        if self.use_last_attn_weights and self.last_attn_slice_weights is not None:
            attn = attn * self.last_attn_slice_weights
            self.use_last_attn_weights = False
        
        # Weight the attention head
        if self.weight_attn_head and self.attn_head_weights is not None:
            attn = attn * self.attn_head_weights
            self.weight_attn_head = False
        ...
        if self.weight_out and self.attn_out_weights is not None:
            out = out * self.attn_out_weights
            self.weight_out = False
        ...
        if self.return_attn:
            ...
            return self.to_out(out), {"q": q, "k": k, "v": v}
