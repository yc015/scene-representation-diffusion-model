...
class UNetMidBlock2DCrossAttn(nn.Module):
    ...
    def forward(self, hidden_states, temb=None, encoder_hidden_states=None):
        ...
        for attn, resnet in zip(self.attentions, self.resnets[1:]):
            if self.return_attn:
                hidden_states, block_attn = attn(hidden_states, encoder_hidden_states)
                attns.append(block_attn)
            elif self.return_qkv:
                hidden_states, block_qkv = attn(hidden_states, encoder_hidden_states)
...
class CrossAttnDownBlock2D(nn.Module):
    ...
    def forward(self, hidden_states, temb=None, encoder_hidden_states=None):
        ...
        for resnet, attn in zip(self.resnets, self.attentions):
            if self.return_attn:
                hidden_states = resnet(hidden_states, temb)
                hidden_states, block_attn = attn(hidden_states, context=encoder_hidden_states)
                output_states += (hidden_states,)
                attns.append(block_attn)
            elif self.return_qkv:
                hidden_states = resnet(hidden_states, temb)
                hidden_states, block_qkv = attn(hidden_states, context=encoder_hidden_states)
                output_states += (hidden_states,)
                qkvs.append(block_qkv)
            else:
                hidden_states = resnet(hidden_states, temb)
        ...
        if self.return_attn:
            return hidden_states, output_states, attns
        elif self.return_qkv:
            return hidden_states, output_states, qkvs
