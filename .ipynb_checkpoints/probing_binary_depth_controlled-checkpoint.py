...
def main(args):
    ...
    probe_checkpoints_dir = f"probe_checkpoints/large_syn_dataset_controlled_fully_permuted/at_step_{at_step}/"
    if not os.path.exists(probe_checkpoints_dir):
        os.makedirs(probe_checkpoints_dir)

    probe_accuracy_dir = f"probe_accuracy/large_syn_dataset_controlled_fully_permuted/at_step_{at_step}/"
        ...
        for block_ind in range(i_s, i_e):
            ...
            for prompt_ind, seed_num in zip(prompt_indexes, sample_seeds):
                ...
                image = generate_image(prompt, seed_num, num_inference_steps=15,
                                       net=permuted_unet, tokenizer=tokenizer, text_encoder=text_encoder, scheduler=scheduler, vae=vae)
            ...
            for layer_ind in range(layer_range):
                ...
                for epoch in range(1, max_epoch + 1):
                    ...
                    if test_results[0] < min_loss:
                        ...
                        torch.save(probe.state_dict(), f"probe_checkpoints/large_syn_dataset_controlled_fully_permuted/at_step_{at_step}/segmentation_probe_{layer}.pth")

                # Save the test results at the last epoch for evaluation
                with open(f"probe_accuracy/large_syn_dataset_controlled_fully_permuted/at_step_{at_step}/saved_test_results_{block}_{block_ind}_layer_{layer_ind}_{postfix}.pkl", "wb") as outfile:
                    pickle.dump(test_results[1], outfile)

                # Save the probe model's weights
                torch.save(probe.state_dict(), f"probe_checkpoints/large_syn_dataset_controlled_fully_permuted/at_step_{at_step}/segmentation_probe_{layer}_final.pth")
                ...
                plt_test_results(probe, 
                                 ...
                                 loss_func, head=None,
                                 save_plt=True,
                                 save_filename=f"probe_accuracy/large_syn_dataset_controlled_fully_permuted/at_step_{at_step}/saved_test_results_{block}_{block_ind}_layer_{layer_ind}_{postfix}.png")
