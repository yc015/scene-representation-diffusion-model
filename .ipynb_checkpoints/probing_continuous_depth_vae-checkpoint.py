...
def main(args):
    ...
    probe_checkpoints_dir = f"probe_checkpoints/large_syn_dataset_vae_continuous_step/at_step_{at_step}/"
    if not os.path.exists(probe_checkpoints_dir):
        os.makedirs(probe_checkpoints_dir)

    probe_accuracy_dir = f"probe_accuracy/large_syn_dataset_vae_continuous_step/at_step_{at_step}/"
    ...
    for block in ["mid", "up"]:
        ...
        for block_ind in range(i_s, i_e):
            ...
            for layer_ind in range(layer_range):
                ...
                with open(f"probe_accuracy/large_syn_dataset_vae_continuous_step/at_step_{at_step}/saved_test_results_{block}_{block_ind}_layer_{layer_ind}_{postfix}{weights_postfix}.pkl", "wb") as outfile:
                    pickle.dump(test_results[2], outfile)

                # Save the probe model's weights
                torch.save(probe.state_dict(), f"probe_checkpoints/large_syn_dataset_vae_continuous_step/at_step_{at_step}/regression_probe_{layer}_final{weights_postfix}.pth")
                ...
                plt_test_results_continuous_depth(probe,
                                                  ...
                                                  save_filename=f"probe_accuracy/large_syn_dataset_vae_continuous_step/at_step_{at_step}/saved_test_results_{block}_{block_ind}_layer_{layer_ind}_{postfix}{weights_postfix}.png")
